...
main([Filename]) ->
    ...
    start_amplifiers_for_all_phases([5, 6, 7, 8, 9], [], Program),
...
start_amplifiers_for_all_phases([], Current, Program) ->
    Master = self(),
    Collector = spawn(fun() -> collector(Master) end),
    [ A, _B, _C, _D, E | _ ] = Amplifiers = start_amplifiers(Program, Collector),
    Collector ! {configure, A, E},
...
start_amplifiers(Program, Collector) ->
...
collector(Master) ->
    receive
        {configure, First, ToMonitor} ->
            erlang:monitor(process, ToMonitor),
            collector(Master, First, 0)
    end.

collector(Master, First, LastValue) ->
    receive
        {input, Value} ->
            First ! {input, Value},
            collector(Master, First, Value);
        {'DOWN', _Ref, process, _, _} ->
            Master ! {result, LastValue}
    end.
