...
sum_fallen(Deps) ->
    maps:fold(fun(ID, {_, Supported}, Sum) ->
                      % "- 1" because be provide the disintegrated brick as "gone"
                      % but we need to count the "other" bricks that have fallen
                      Sum + length(fall([ID], Supported, Deps)) - 1
              end, 0, Deps).

fall(Gone, [], _) ->
    Gone;
fall(Gone, Candidates, Deps) ->
    Filtered = lists:filtermap(
                 fun(Candidate) ->
                         {SupportedBy, Supported} = maps:get(Candidate, Deps),
                         case SupportedBy -- Gone of
                             [] -> {true, {Candidate, Supported}};
                             _ -> false
                         end
                 end, Candidates),
    {GoneCandidates, NextCandidates} = lists:unzip(Filtered),
    fall(lists:usort(GoneCandidates ++ Gone), lists:usort(lists:merge(NextCandidates)), Deps).
