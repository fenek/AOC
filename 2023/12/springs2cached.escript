...
solve(Lines, UnfoldMultiplier) ->
    ets:new(spring_cache, [named_table]),
    Output = lists:foldl(fun(Line, Sum) ->
                                 Sum + solve_line(Line)
                         end, 0, parse(Lines, UnfoldMultiplier)),
    ets:delete(spring_cache),
    Output.
...
find_solutions_cached(Springs, Checksums) ->
    case ets:lookup(spring_cache, {Springs, Checksums}) of
        [{_, Total}] ->
            Total;
        _ ->
            Total = find_solutions(Springs, Checksums),
            ets:insert(spring_cache, {{Springs, Checksums}, Total}),
            Total
    end.
