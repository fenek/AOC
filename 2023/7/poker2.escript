...
get_hand_type(Hand) ->
    ...
    {JokerCount, CountsWithoutJoker} = case maps:take(card_to_int($J), Counts) of
                                           error -> {0, Counts};
                                           Result -> Result
                                       end,
    get_type(lists:reverse(lists:sort(maps:values(CountsWithoutJoker))), JokerCount).

% Note: Two pairs can't be achieved with a joker - a joker and a single pair would result in three.
get_type([], 5) -> 7; % five jokers
get_type([A | _], J) when A + J == 5 -> 7; % five of a kind
get_type([A | _], J) when A + J == 4 -> 6; % four of a kind
get_type([A, 2 | _], J) when A + J == 3 -> 5; % full house
get_type([A | _], J) when A + J == 3 -> 4; % three of a kind
get_type([2, 2, 1], 0) -> 3; % two pairs
get_type([A | _], J) when A + J == 2 -> 2; % pair
get_type(_, _) -> 1. % high card
