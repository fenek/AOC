...
solve(Lines) ->
    {RowSum, ColSum} = lists:foldl(fun(Puzzle, {RowSum0, ColSum0}) ->
                                           {RowRefs, ColRefs} = find_reflections_in_puzzle(Puzzle),
                                           {RowSum0 + lists:sum(RowRefs),
                                            ColSum0 + lists:sum(ColRefs)}
                                   end, {0, 0}, parse(Lines)),
    ColSum + 100 * RowSum.

find_reflections_in_puzzle({Rows, Cols}) ->
    RowRefs = find_reflection(Rows),
    ColRefs = find_reflection(Cols),
    {RowRefs, ColRefs}.
...
find_reflection(N, Max, Sequence) ->
    case is_reflection(N, N + 1, Max, Sequence) of
        ...
        true ->
            [N | find_reflection(N + 1, Max, Sequence)]
    end.

is_reflection(N1, N2, Max, _Sequence) when N1 < 1; N2 > Max ->
    true;
is_reflection(N1, N2, Max, Sequence) ->
    #{ N1 := Val1, N2 := Val2 } = Sequence,
    case Val1 == Val2 of
        true ->
            is_reflection(N1 - 1, N2 + 1, Max, Sequence);
        false ->
            false
    end.
