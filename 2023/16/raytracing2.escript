...
solve([Line | _] = Lines) ->
    ...
    MaxX = byte_size(Line),
    MaxY = length(Lines),
    Contraption = Contraption0#{ max_x => MaxX, max_y => MaxY },
    Rays = lists:flatmap(fun(X) ->
                                 [init_attempt(Contraption, X, 1, down),
                                  init_attempt(Contraption, X, MaxY, up)]
                         end, lists:seq(1, MaxX))
        ++ lists:flatmap(fun(Y) ->
                                 [init_attempt(Contraption, 1, Y, right),
                                  init_attempt(Contraption, MaxY, Y, left)]
                         end, lists:seq(1, MaxY)),
    Results = collect_all(Rays),
    lists:max(Results).

init_attempt(Contraption, X0, Y0, Dir) ->
    Energized = ets:new(energized, [public, bag]),
    Ray = send_ray(Contraption, X0, Y0, Dir, Energized),
    {Energized, Ray}.

collect_all(Rays) ->
    lists:map(fun({Energized, Ray}) ->
                      collect_ray(Ray),
                      Result = sets:size(sets:from_list([ element(1, T)
                                                          || T <- ets:tab2list(Energized) ])),
